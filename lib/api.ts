...
export type Project = { id: number; name: string; slug: string; repositoryUrl: string; branch: string; runtime: Runtime; applicationPort: number; domain: string | null; autoDeploy: boolean; createdAt: string };
...
export type Server = { id: number; name: string; status: string; cpu: number; ram: number; disk: number; containers: number; lastHeartbeat: string };
...
export const api = {
  ...
  dashboard: () => request<Dashboard>("/dashboard"), monitoring: () => request<Monitoring>("/monitoring"), servers: () => request<Server[]>("/servers"), connections: () => request<GitConnection[]>("/git-connections"),
};
